...
public class MultitoolInteractionsEvent {
    public static InteractionResult execute(UseOnContext context, int mode) {
        ...
        Optional<BlockState> optional1 = optional.isPresent() ? Optional.empty() : Optional.ofNullable(state.getToolModifiedState(context, ItemAbilities.AXE_SCRAPE, false));
        Optional<BlockState> optional2 = optional.isEmpty() && optional1.isEmpty() ? Optional.ofNullable(state.getToolModifiedState(context, ItemAbilities.AXE_WAX_OFF, false)) : Optional.empty();
        ...
        if (mode == 0) {
            if (context.getClickedFace() == Direction.DOWN) {
                ...
                BlockState state2 = state.getToolModifiedState(context, ItemAbilities.SHOVEL_FLATTEN, false);
