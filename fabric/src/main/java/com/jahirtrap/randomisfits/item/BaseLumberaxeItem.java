...
public class BaseLumberaxeItem extends AxeItem {
    ...
    public BaseLumberaxeItem(Tier tier, Properties properties) {
        super(tier, 6, -3f, properties.tab(TAB_RANDOMISFITS));
    }
    ...
    private String getModeText(Boolean felling) {
        String mode;
        if (felling) mode = new TranslatableComponent("randomisfits.lumberaxe.felling.yes").getString();
        else mode = new TranslatableComponent("randomisfits.lumberaxe.felling.no").getString();
